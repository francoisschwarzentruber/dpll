...
{
	UIpostMessage({gui: true, printConsole: true,  string: string});
...
function getClausesTableString(state)
{
	...
	s += (getPrettyPrintFromClauseWithNumber(state.clauses[i]));
...
function constructImplicationGraph(state, containerName, conflictClause)
{
  ...
  UIpostMessage({gui: true, newGraph: true, containerName: containerName, data: data});
...
function UIDPLL_init()
{
	...
	printConsole("<h1>Details of the execution</h1>");
	numstep = 0;
